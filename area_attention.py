...
class AreaAttention(nn.Module):
    ...
    def __init__(
        ...
        :param memory_height: memory height for arranging features into a grid 这个参数主要用于将特征重新排列为网格形式。
        :param memory_width: memory width for arranging features into a grid
        :param dropout_rate: dropout rate
        :param top_k_areas: top key areas used for attention 这个参数用于指定在进行注意力计算时，选择注意力分数最高的前k个区域。
        """
        ...
        if self.area_key_mode in ['concat', 'max_concat', 'sum', 'sample_concat', 'sample_sum']:
            ...
            if area_key_mode == 'concat':
                area_key_feature_size = 3 * key_query_size # 把表示一个区域特性的键(key)，转化成神经网络可以处理的嵌入(embedding)。
